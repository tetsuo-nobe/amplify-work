...
複数の受講者が同一の AWS アカウントを共有する場合もあるため、リソース名が重複しないように、ユニークな **ユーザー名** を決めてください。

本手順では以下のように表記します。実際の操作では `{ユーザー名}` の部分をご自身のユニークなユーザー名に置き換えてください。

- CodeCommit リポジトリ名: `next-hello-ts-{ユーザー名}`
- 例: IAM ユーザー名が `user01` の場合 → `next-hello-ts-user01`
...
4. **「リポジトリを作成」** をクリックします。
5. 以下の設定でリポジトリを作成します。
   - リポジトリ名: `next-hello-ts-{ユーザー名}`（例: `next-hello-ts-user01`）
